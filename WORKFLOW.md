...
## PDF Export Workflow (July 2025)
- The repository includes a clean Markdown-to-Word converter. To generate a PDF, first use `convert_to_word.py` to create a `.docx` file, then open the file in Microsoft Word and use "Save as PDF".
...
- LaTeX templates are available in the `/templates/` folder for users who prefer LaTeX compilation, but are not integrated into the automated workflow.
...
5. **Assembly and Export**
   ...
   - Export to .docx using the conversion script (`convert_to_word.py`).
